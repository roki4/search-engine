/**
 * @fileoverview Search route for performing Google Custom Search and document search.
...
/**
 * Perform a document search (PDF, TXT, DOCX) using Google Custom Search API.
 * @name GET/api/document-search
 * @function
 * @param {Object} req - Express request object.
 * @param {string} req.query.q - Search query string.
 * @param {number} [req.query.start=1] - Start index for pagination.
 * @param {Object} res - Express response object.
 * @returns {Object} 200 - Document search results and total count.
 * @throws {Object} 400 - If query or start is invalid.
 * @throws {Object} 500 - Server error.
 */
router.get('/document-search', validate(searchSchema), async (req, res) => {
  try {
    const { q, start = 1 } = req.query;
    ...
    // Combine query with file type filters
    const docQuery = `${q} (filetype:pdf | filetype:txt | filetype:docx)`;
    ...
    const results = response.data.items
      ? response.data.items.map((item) => ({
          ...
          url: item.link,
          snippet: item.snippet,
          fileType: item.fileFormat ? item.fileFormat.split('/').pop().toUpperCase() : 'UNKNOWN',
        }))
      : [];
